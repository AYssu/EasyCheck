<template>
  <div>
    <div style="margin-top: 10px">
      ...
      >
        ...
        <el-form-item v-if="!phone_bool">
          <el-button style="color: white" :icon="FolderAdd" color="#42b983" @click="drawer = true"
            >添加项目</el-button
          >
        </el-form-item>
      </el-form>
      ...
      >
          ...
          <a-dropdown :trigger="['contextmenu', 'click']" placement="bottomLeft">
            ...
            <template #overlay>
              <a-menu>
                ...
                <a-menu-item
                  key="3"
                  @click="handle_edit_variable(item.projectId, item.projectName)"
                >
                  <el-text style="color: grey" size="small">
                    <el-icon style="color: #515151; margin-right: 3px">
                      <svg
        ...
        <a-card-meta>
          ...
          <template #description>
            <div>
              <el-text
                :style="phone_bool ? 'font-size: 0.7rem' : 'font-size: 0.9rem'"
                size="small"
                type="info"
              >
        ...
        <a-form-item>
          <template #label>
            ...
            <el-icon
              ...
              @click="click_reset_key(show_project_info.projectId)"
            >
              <svg
                class="icon"
                height="200"
    ...
    <a-modal
      v-model:open="json_show"
      :closable="true"
      :title="choose_project_variable.title"
      :width="phone_bool ? '' : '60%'"
    >
      <Codemirror
        ref="cmRef"
      ...
      <template #footer>
        <el-row justify="end">
          ...
          <el-button
            :loading="update_variable_loading"
            color="#17926c"
            type="primary"
            @click="update_variable"
          >
            更新变量
          </el-button>
        </el-row>
    ...
    <a-modal v-model:open="show_project_update" title="编辑项目更新">
      <template #footer>
        ...
        <a-button
          :loading="project_update_loading"
          type="primary"
          @click="update_project_update_info"
          >修改</a-button
        >
      </template>
      <a-form :model="project_update_info">
        <a-form-item label="更新版本" style="margin-top: 20px">
          <a-input
            v-model:value="project_update_info.updateVersion"
            placeholder="请输入项目名称"
          ></a-input>
        </a-form-item>
        ...
        <a-form-item label="更新链接">
          <a-input
            v-model:value="project_update_info.updateUrl"
            placeholder="请输入项目更新链接"
          ></a-input>
        </a-form-item>
          ...
          <div style="display: flex">
            ...
            >
              <template #header>
                <el-checkbox
                  style="
                    --el-checkbox-checked-bg-color: #17926c;
                    --el-checkbox-checked-text-color: #17926c;
                  "
                  v-model="checkAll"
      ...
      <el-table :data="data" size="small">
        ...
        <el-table-column label="接口地址" prop="link" width="130">
          <template #default="scope">
            <el-link
              ...
              type="success"
              @click="copy_text('/' + scope.row.link)"
            >
              /{{ scope.row.link }}
            </el-link>
          </template>
        ...
        <el-table-column label="操作" width="140">
          <template #default="scope">
            ...
            <el-button plain size="small" type="danger" @click="link_delete_click(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
        ...
        contentString
          ...
          .map(function (line: any, index: any) {
            ...
            return createVNode(
              'span',
              {},
              line + (index < contentString.split('\n').length - 1 ? '\n' : ''),
            )
          })
          .flatMap((node: any) => {
            // 如果不是最后一行，添加一个 br 元素
